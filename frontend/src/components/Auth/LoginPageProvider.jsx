        ...
        <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50 flex items-center justify-center py-12 px-4">
            <div className="max-w-md w-full">
                ...
                <button
                    ...
                    className="flex items-center text-green-600 hover:text-green-800 font-semibold mb-8"
                    type="button"
                >
                ...
                <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            ...
                            <input type="email" value={formData.email} onChange={(e) => handleInputChange('email', e.target.value)} className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors ${errors.email ? 'border-red-500 bg-red-50' : 'border-gray-300'}`} placeholder="votre@email.com" disabled={isLoading} />
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input type={showPassword ? 'text' : 'password'} value={formData.password} onChange={(e) => handleInputChange('password', e.target.value)} className={`w-full px-4 py-3 pr-12 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors ${errors.password ? 'border-red-500 bg-red-50' : 'border-gray-300'}`} placeholder="Votre mot de passe" disabled={isLoading} />
                        ...
                        <div className="text-right">
                            <button type="button" onClick={() => alert('Mot de passe oublié')} className="text-sm text-green-600 hover:text-green-700 hover:underline" disabled={isLoading}>Mot de passe oublié ?</button>
                        </div>

                        <button type="submit" disabled={isLoading} className={`w-full py-3 px-4 rounded-lg font-semibold text-white transition-all duration-200 flex items-center justify-center ${isLoading ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-blue-700 hover:shadow-lg transform hover:scale-105'}`}>
                            {isLoading ? (<><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin mr-2"></div>Connexion en cours...</>) : (<>Se connecter <ArrowRight className="w-5 h-5 ml-2" /></>)}
                        </button>
                    </form>
                </div>
                ...
                <div className="text-center mt-8">
                    <p className="text-gray-600">
                        ...
                        <button onClick={() => navigate('/auth/register/provider')} className="text-green-600 hover:text-green-700 font-semibold hover:underline">Créer un compte provider</button>
                    </p>
                </div>
                ...
                <div className="mt-8 text-center">
                    <div className="inline-flex items-center p-3 bg-green-50 rounded-lg border border-green-200">
                        ...
                        <span className="text-sm text-green-700 font-medium">Espace Prestataire de lavage</span>
                    </div>
                </div>
            </div>
        </div>
